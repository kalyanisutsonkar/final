(function ($) {
  ...
  $documentOn.ready(function () {
    ...
    if ($(".gt-testimonial-box-slider-2").length > 0) {
      const gtTestimonialBoxSlider2 = new Swiper(
        ".gt-testimonial-box-slider-2",
        {
          slidesPerView: "auto",
          spaceBetween: 20,
          freemode: true,
    ...
    if (
      $(".th-testimonial-slider").length > 0 &&
      $(".swiper-image").length > 0
    ) {
    ...
    document.querySelectorAll(".gt-coming-soon-time").forEach((container) => {
      ...
      const updateCountdown = () => {
        ...
        if (diff <= 0) {
          clearInterval(timer);
          container.innerHTML =
            "<p class='countdown-ended'>Countdown has ended!</p>";
          return;
        }
      ...
      quantityButtons.forEach((button) => {
        button.addEventListener("click", function () {
          ...
          if (button.classList.contains("quantityIncrement")) {
            ...
            button.classList.contains("quantityDecrement") &&
            value > 1
          ) {
            input.value = value - 1;
          }
    ...
    $(".checkout-radio-single input[name='pay-method']").on(
      "change",
      function () {
        updatePaymentMethod();
      }
    ...
    .forEach((button, index, buttons) => {
      button.addEventListener("click", function (e) {
        ...
        if (targetCollapse.classList.contains("show")) {
          ...
          if (nextButton) {
            const nextCollapse = document.querySelector(
              nextButton.dataset.bsTarget
            );
            const nextInstance =
              bootstrap.Collapse.getOrCreateInstance(nextCollapse);
            nextInstance.show();
          }
        }
      });
